...
function updateStats() {
  ...
  // progress bar
  progressBar.style.width = `${Math.min(100, Math.round((typed.length / totalChars) * 100))}%`;
  ...
  // auto-finish
  if (typed.length >= totalChars && !finished) finishTest();
}
...
document.getElementById('change-sample').addEventListener('click', () => {
  ...
  const pick = other[Math.floor(Math.random() * other.length)] || SAMPLES[0];
  setPassage(pick);
});
